...
export function renderResultsPage(entries, config = {}, { nowMs = Date.now() } = {}) {
  ...
  const resultRows = safeEntries.map((entry, orderIndex) => {
    ...
    return `<tr class="result-row" data-id="${escapeHtml(itemId)}" data-series="${escapeHtml(entry.seriesId ?? '')}" data-changed="${changes.length ? 1 : 0}" data-triage="unseen" data-order="${orderIndex}" data-grade="${gradeRank}" data-price="${hasPrice ? entry.price : ''}" data-decision="${decisionRank}" data-likes="${Number.isInteger(entry.likeCount) ? entry.likeCount : ''}" data-condition="${Number.isInteger(entry.itemConditionLevel) ? entry.itemConditionLevel : ''}" data-title="${escapeHtml(entry.title)}" data-match="${shouldAlert ? 1 : 0}" data-budget="${isWithinBudget ? 1 : 0}" data-new="${isRecent ? 1 : 0}" data-published="${entry.publishedAt ? Date.parse(entry.publishedAt) : ''}" data-search="${escapeHtml(searchable)}">
      <td class="grade-cell" aria-label="${escapeHtml(entry.grade)}ランク"><span class="grade grade-${escapeHtml(entry.grade)}" aria-hidden="true">${escapeHtml(entry.grade)}</span></td>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    table { width: 100%; min-width: 1280px; table-layout: fixed; border-collapse: separate; border-spacing: 0; }
    th:nth-child(1) { width: 78px; }
    th:nth-child(2) { width: 104px; }
    th:nth-child(3) { width: 172px; }
    ...
    td { padding-top: 15px; padding-bottom: 15px; color: #34373c; font-size: 13px; }
    ...
    .condition-text { display: block; overflow: hidden; color: #55534e; text-overflow: ellipsis; white-space: nowrap; }
    .product-cell { width: 100%; min-width: 440px; white-space: normal; }
    .product-main { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 16px; min-width: 0; }
    ...
    .product-facts { display: block; min-width: 0; margin-top: 5px; overflow: hidden; color: #817b72; font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
    .product-actions { display: inline-flex; flex: 0 0 auto; align-items: center; gap: 8px; padding-left: 14px; border-left: 1px solid #e5ddcf; white-space: nowrap; }
