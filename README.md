...
查看可点击结果：双击 `open-results.cmd`。它会在需要时自动启动隐藏的后台监测器；如需停止，双击 `stop-background.cmd`。浏览器结果页中的商品标题可直接跳转到メルカリ，页面每5分钟刷新一次。页头状态灯每10秒读取一次轻量心跳：绿色表示后台正常运行，闪动金色表示正在搜索或更新，灰红色表示没有启动或心跳中断；这不会刷新整张商品表。后台默认每10分钟复查全部商品的当前价格、いいね数、商品状态及是否已售出，并使用3个页面并行缩短等待时间。いいね数字旁的绿点表示最近更新成功，金点表示正在等待刷新；鼠标停留可查看最后更新时间。
