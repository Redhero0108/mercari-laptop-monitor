...
查看可点击结果：双击 `open-results.cmd`。它会在需要时自动启动隐藏的后台监测器；如需停止，双击 `stop-background.cmd`。浏览器结果页中的商品标题和“打开商品”按钮都可直接跳转到メルカリ，并且每30秒自动刷新。后台默认每10分钟复查全部商品的当前价格、いいね数、商品状态及是否已售出，并使用3个页面并行缩短等待时间。いいね数字旁的绿点表示最近更新成功，金点表示正在等待刷新；鼠标停留可查看最后更新时间。点击“等级、价格、いいね数、商品状态、商品、判断、发布时间、检查时间”可切换升序／降序，排序选择会自动保留。页面只显示S/A/B/C等级，原始评分仅在程序内部用于筛选和排序。
