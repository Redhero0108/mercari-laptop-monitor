...
当新商品达到阈值时，Windows会弹出提示；选择“是”即可打开商品页面。已记录的商品若之后降价到预算内且满足提醒条件，也会再次弹出降价提醒。程序只读取公开商品页面，不登录、不留言、不收藏、更不会自动购买。
...
- `detailCheckLimit`：每轮最多打开的新增商品详情数，默认30、最高80。
...
- `searchConcurrency`：搜索关键词时同时使用的页面数，默认3、最高5。
- `headless`：`true`表示浏览器在后台运行。
- `notify`：是否显示Windows通知。
- `queries`：メルカリ搜索关键词。
- `wideQueries`：宽搜索关键词（不含“32GB”），用于召回标题未写内存、但详情页包含32GB的商品；宽搜索结果同样会打开详情页，经评分与硬过滤后才进入结果列表。
